...
if (isset($_POST["submit"])) {
    ...
    $sql = "INSERT INTO `Trust` (`Name`, `ID`, `Location`) VALUES ('$name', '$id', '$location')";

    $result = mysqli_query($conn, $sql);

    if ($result) {
        header("Location: trust_index.php?msg=New record created successfully");
    } else {
...
<nav class="navbar navbar-light justify-content-between fs-3 mb-5" style="background-color: #00ff5573;">
    ...
    <div class="container">
        <div class="text-center mb-4">
            <h3>Add New Trust Entry</h3>
            <p class="text-muted">Complete the form below to add a new trust entry</p>
        </div>
        ...
        <div class="container d-flex justify-content-center">
            <form action="" method="post" style="width:50vw; min-width:300px;">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="name" placeholder="Trust Name">
                </div>


                <div class="mb-3">
                    <label class="form-label">Location:</label>
                    <input type="text" class="form-control" name="location" placeholder="Trust Location">
                </div>

                <div>
                    <button type="submit" class="btn btn-success" name="submit">Save</button>
                    <a href="Trust_index.php" class="btn btn-danger">Cancel</a>
                </div>
            </form>
        </div>
    </div>
